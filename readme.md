...
```
    ...
    <matplotlib.axes._subplots.AxesSubplot at 0x2e005e199e8>
...
class NeuralNetwork(object):
    def __init__(self, input_nodes, hidden_nodes, output_nodes, learning_rate):
        ...
        self.sigmoid_prime_function = sigmoid_prime
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            hidden_error_term = hidden_error * self.sigmoid_prime_function(hidden_inputs)
...
```
    ...
    Progress: 100.0% ... Training loss: 0.099 ... Validation loss: 0.186
...
Even that we have a larger losses, the learning curve error is smoother than before and our model and the effect is positive on generalization and predictive abilities of our model. But with this configuration our iterations were not enough to have our model well trained so we configured a new set of hyperparameters using a smaller learning rate to avoid large weight steps and increased our iterations untill we were able to identify a slight plateau on the Learning error curve:
...
output_nodes = 1
```
We also took care to keep our validation loss between 0.20 and 0.15 to make our data fit well without excessive execution.
...
```
conda create -n bsnn --file bike-sharing-nn-env.yml python=3
```
Or if you need to import it to a previously created environment, you can use the command:
